...
size_t IndexBuffer::size() const { return count; }
...
VertexArray::VertexArray(
  ...
  for (const auto& buf : *(this->vertexBuffers)) {
    ...
    for (size_t i = 0; i < layout.size(); ++i) {
      ...
      glVertexAttribPointer(i, layout[i].numberOfComponents,
                            OpenGL::toOpenGL(layout[i].type),
                            layout[i].normalize ? GL_TRUE : GL_FALSE,
                            layout.getStride(), (const void*)layout[i].offset);
    }
  }
...
const std::shared_ptr<std::vector<Trundle::VertexBuffer>>&
VertexArray::getVertexBuffers() const {
  return vertexBuffers;
}

const std::shared_ptr<Trundle::IndexBuffer>&
