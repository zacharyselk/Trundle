...
// A test class that keeps track of the work done to verify that threads are
// finishing.
class TestThread {
public:
  ...
  void run() { result = Trundle::math::fibonacci_linear(index); }
...
bool verifyThreads() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (tests[i].getResult() != result) {
      printf("Error: incorrect result, thread calcualted fib(%i) = %i and not "
             "%i\n",
