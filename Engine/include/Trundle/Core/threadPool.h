...
public:
  ...
  // otherwise the next function will be put into func and true will be
  // returned.
  ...
  // Attempt to pop the next item off of the queue, returning false if the pop
  // failed.
  ...
  template <typename T> bool tryPush(T&& func) {
    {
      // Attempt to lock the queue.
      std::unique_lock<std::mutex> lock{mutex, std::try_to_lock};
      if (!lock) {
...
public:
  ...
  template <typename T> void async(T&& func) {
    auto n = index++;
